...
class Home:
    def __init__(self, logged_in_account):
        ...
        self.accounts = self.load_accounts(False)
    ...
    def delete_acc(self):
        confirm = input('Are you sure you want to remove your account [y/n]? ').lower()
        if confirm == 'y':
            print('\nPlease type the email and password to continue')
            email = input('Email: ')
            password = input('Password: ')
            if email == self.account.get_email() and password == self.account.get_password():
                print('Thanks for working with us, deleting your account...')
                time.sleep(2)
                all_accounts = self.load_accounts(True)  # Have to make a new list that doesn't exclude this account
                for account in all_accounts:
                    if self.account.get_account_num() == account.get_account_num():
                        all_accounts.remove(account)
                        with open('accounts.txt', 'wb') as accounts_file:
                            pickle.dump(all_accounts, accounts_file)
                        break
                print('Your account has been deleted, have a nice day')
                os.system('cls' if os.name == 'nt' else 'clear')
                os.system('python main.py')   
        else:
            print('\nSending you back home home page...')
            time.sleep(2.5)
            self.clear('')
